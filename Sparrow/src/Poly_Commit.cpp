...
void MIPP_gen_stream(MIPP_Comm &commitment){
	...
	commitment.bit_size = 2*(int)log2(MAX_CHUNCK_COMM);
	//commitment.commit_bit_size = chunck_size;
 	commitment.commit_bit_size = (int)(log2(MAX_CHUNCK_COMM));
...
void generate_streaming_pp(struct Comm &commitment,int bit_size){
	...
	int K = (1ULL<<bit_size)/MAX_CHUNCK_COMM;
	vector<vector<int>> idx(MAX_CHUNCK_COMM);
...
void generate_pp(struct Comm &commitment,int bit_size){
	...
	for(int i = bit_size-2; i >= 0; i--){
		for(int j = 0; j < commitment.pp1_cached[i+1].size()/2; j++){
			//commitment.pp1_cached[i].push_back(commitment.pp1_cached[i+1][j] + commitment.pp1_cached[i+1][j+commitment.pp1_cached[i+1].size()/2]);
...
void MIPP_commit_stream(stream_descriptor fd, int size, MIPP_Comm &commitment, vector<G1> &commitments){
	...
	printf("%d,%d\n",MAX_CHUNCK_COMM,size);
	if(MAX_CHUNCK_COMM >= size){
		vector<F> buff(size);
		...
		clock_t t1,t2;
		t1 = clock();
		vector<F> buff(MAX_CHUNCK_COMM);
		for(int i = 0; i < size/MAX_CHUNCK_COMM; i++){
			read_stream(fd,buff,MAX_CHUNCK_COMM);
...
void MIPP_open_stream(stream_descriptor fd, int size,vector<F> x, MIPP_Comm &commitment, vector<G1> commitments){
	...
	if(size > MAX_CHUNCK_COMM){
		...
		for(int i = 0; i < size/MAX_CHUNCK_COMM; i++){
			read_stream(fd,buff,MAX_CHUNCK_COMM);
			for(int j = 0; j < MAX_CHUNCK_COMM; j++){
		...
		*/
		...
		int offset = (int)log2(MAX_CHUNCK_COMM) - (int)log2(buff.size());
		commitment.Proof.clear();
		G1 P;
