...
prover::prover(const layeredCircuit &cir,char *filename,vector<F> data, bool d): C(cir), proof_size(0) {
    debug = d;
    evaluate(filename ,data);
...
void prover::evaluate(char *filename,vector<F> data)
{
        ...
        for (u64 g = 0; g < C.circuit[i].size; ++g)
        {
            ...
            switch (ty) {
                ...
                case gateType::AddMul:
                    //char buff[256];
                    //circuitValue[i][g].getStr(buff,256,10);
                    //printf("%s\n",buff );
                    circuitValue[i][g] = F(0);
 ...
 */
F prover::Vres(const vector<F>::const_iterator &r_0, int r_0_size, vector<F> &out)
{
    ...
    if(debug){
        ...
        //for(int i = 0; i < circuitValue[C.size-1].size(); i++){
            ...
            if(circuitValue[C.size-1][i] != F(0)){
                printf("%s\n",buff );
                printf("%d\n", i);
            }
            //std::cout << num << "\n";
        ...
        printf(">%d\n",circuitValue[C.size-1].size() );

    }
    out = circuitValue[C.size-1];
   //printf(">%d\n",circuitValue[C.size-1].size() );
