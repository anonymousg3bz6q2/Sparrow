...
proof verifier::verify(vector<F> randomness, F previous_r)
{
#ifdef USE_VIRGO
    ...
    vector<F> input;
	F previousSum = p->Vres(r_0, C.circuit[C.size - 1].bitLength, P.output);
    
    
    input.push_back(randomness[randomness.size()-1]);
    input.push_back(previousSum);
    F previous_random = previous_r;
    ...
    //printf("%lf\n",proving_time );
    verification_time += verifyTime();
    proof_len += (int)(1024.0*p->proofSize());
...
bool verifier::verifyPhase1(int layer_id, F &previousSum, F &previousRandom) {
    ...
    F assert_random;
    assert_random.setByCSPRNG();// = F::random();
    //char buff[256];
    ...
    for (int j = 0; j < pre.bitLength; ++j)
    {
        ...
        vector<F> input;
        previousRandom = mimc_hash(previousRandom,poly.a);
        previousRandom = mimc_hash(previousRandom,poly.b);
        previousRandom = mimc_hash(previousRandom,poly.c);
...
bool verifier::verifyPhase2(int layer_id, F &previousSum, F &previousRandom) {
    ...
    auto &cur = C.circuit[layer_id], &pre = C.circuit[layer_id - 1];
    for (auto &x: r_v[layer_id]) x.setByCSPRNG();// = F::random();
...
bool verifier::verifyLiu(int layer_id, F &previousSum, F &previousRandom) {
    ...
    for (auto &x: sig) x.setByCSPRNG();// = F::random();
    for (auto &x: r_liu) x.setByCSPRNG();//x = F::random();
    ...
    for(int i = 0; i < tsig.size(); i++){
        sum += tsig[i]*tfinal_claims_v[i];
    }
    

    previousRandom = mimc_hash(previousRandom,sum);
    //verify_timer.stop();
    verify_slow_timer.stop();
    ...
    P.vr.push_back(vr);
    P.gr.push_back(gr);
    previousRandom = mimc_hash(previousRandom,vr);
    previousRandom = mimc_hash(previousRandom,gr);
